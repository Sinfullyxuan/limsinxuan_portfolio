const contactSketch = (p) => {
  ...
  // ✅ cap connections per node (prevents dense web on mobile)
  let maxLinksPerNode = 6;

  // ✅ cached animated gradient
  let bg = null;
  let needsBg = true;
  const GRADIENT_EVERY = 3; // rebuild every 3 frames
  ...
  function setResponsiveParams() {
    ...
    // scale distances with canvas (prevents “condensed” look)
  ...
  function buildAnimatedGradient() {
    ...
    // ✅ your sketch4 gradient colors (swapped vs sketch3)
  ...
  p.setup = () => {
    ...
    const h = (section.clientHeight || section.offsetHeight) + 1;

    const canvas = p.createCanvas(w, h);
    canvas.parent("projects-2-sketch"); // ✅ correct parent
    canvas.position(0, 0);
